...
set called `heart_scale.t`, then type `svm-predict heart_scale.t heart_scale.model output` to see the prediction accuracy. The `output`
...
`svm-predict.c` for examples showing how to use them. We define LIBSVM_VERSION and declare `extern int libsvm_version`;  in svm.h, so
